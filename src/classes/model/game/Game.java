...
/**
 * A class representing the game "C'est du brutal".
 * How to use it:
 * <pre>
...
public class Game {
    ...
    static final String[] firstNames = {"Laura", "Miki", "Harry", "James", "Gregory", "Emily", "Curtis", "Damian", "Steven", "Daniel", "John", "Sara", "Mary", "Christina", "Mario", "Lola", "Katharine", "Cynthia", "Brenda", "Timothy"};
    static final String[] lastNames = {"Barrett", "Ryan", "Matthews", "Sparks", "Cordoba", "Reynolds", "Zieman", "Johnson", "Capdeville", "Srour", "Lundberg", "Ryan", "Amidon", "Klumpp", "Roper", "Piatkowski", "Aguilar", "Gray", "Woods", "Swisher"};
    static final String[] districtNames = {"Library", "Student office", "Administrative district", "Industrial halls", "Sports hall"};
    private Player p1;
    private Player p2;
    ...
    /**
     * Generate an array of districts, initialized with the names defined in the class.
     *
     * @see District
    ...
    private ArrayList<Student> generateTeam() {
        ...
        for(int i=0; i<nStudent; i++) {
            f.add(new Student(firstNames[rand.nextInt(firstNames.length)], lastNames[rand.nextInt(lastNames.length)]));
        }

        for(int i=0; i<nElite; i++) {
            f.add(new EliteStudent(firstNames[rand.nextInt(firstNames.length)], lastNames[rand.nextInt(lastNames.length)]));
        }

        for(int i=0; i<nMaster; i++) {
            f.add(new GobiMaster(firstNames[rand.nextInt(firstNames.length)], lastNames[rand.nextInt(lastNames.length)]));
        }
    ...
    /**
     * Check if the game is over (if 1 player controls 3 or more districts), if so, the winner and the score is displayed.
     *
    ...
    public int getScoreP1() {
        int score = 0;
        for (District d: this.districts) {
            if (d.getTeam(this.p2.getTeam()).isEmpty() && !d.getTeam(this.p1.getTeam()).isEmpty()) score++;
        }
        return score;
    }
    ...
    public int getScoreP2() {
        int score = 0;
        for (District d: this.districts) {
            if (d.getTeam(this.p1.getTeam()).isEmpty() && !d.getTeam(this.p2.getTeam()).isEmpty()) score++;
        }
        return score;
    }
    ...
    public District[] getActiveDistricts() {
        ...
        for (District d: this.districts) {
            if (d.isFightOver() == Team.NONE) temp.add(d);
        }
    ...
    public String toString() {
        return "Game{" +
                ...
                ", districtNames=" + Arrays.toString(districtNames) +
                ", p1=" + p1 +
                ", p2=" + p2 +
